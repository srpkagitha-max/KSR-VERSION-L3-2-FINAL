...
// L5.1 SMART IMPORT ENGINE PART-1
function ksrIsHeadingLine(line){
  const s=String(line||"").trim();
  if(!s) return false;
  if(/^\*[^*]{2,60}\*$/.test(s)) return true;
  if(/^#+\s+/.test(s)) return true;
  if(/^(App\s+Exam|App\s+GK|App\s+PIE|App\s+Psychology|Psychology|Telugu|English|GK|Maths?|Science)$/i.test(s.replace(/\*/g,"").trim())) return true;
  return false;
}
function ksrHeadingText(line){
  return String(line||"").replace(/\*/g,"").replace(/^#+\s*/,"").trim();
}
function ksrAutoFixImportText(raw){
  let subject="General", out=[], qCounter=0;
  const lines=String(raw||"").replace(/\r/g,"\n").split("\n").map(x=>x.trim()).filter(Boolean);

  for(let line of lines){
    if(ksrIsHeadingLine(line)){
      subject=ksrHeadingText(line)||subject;
      out.push("Subject: "+subject);
      continue;
    }
    line=line
      .replace(/^[Qq]\s*(\d+)\s*[\.\)]?\s*/,"$1. ")
      .replace(/^([ABCD])\s*[\)\-:]\s*/i,(m,p)=>p.toUpperCase()+". ")
      .replace(/^(జవాబు|సమాధానం)\s*[:：\-]?\s*/i,"Answer: ")
      .replace(/^Ans(?:wer)?\s*[:：\-]?\s*/i,"Answer: ");
    out.push(line);
  }

  // Add missing question numbers when A/B/C/D groups appear without number
  let fixed=[], pendingQuestion=[];
  function flushPending(){
    if(pendingQuestion.length){
      qCounter++;
      fixed.push(qCounter+". "+pendingQuestion.join(" "));
      pendingQuestion=[];
    }
  }

  for(const line of out){
    if(/^Subject\s*[:：]/i.test(line)){ flushPending(); fixed.push(line); continue; }
    if(/^\d+[\.\)]\s+/.test(line)){
      flushPending();
      qCounter = Math.max(qCounter, Number((line.match(/^(\d+)/)||[])[1])||qCounter);
      fixed.push(line.replace(/^(\d+)[\.\)]/,"$1."));
      continue;
    }
    if(/^[ABCD]\s*[\.\)]\s*/i.test(line) || /^Answer\s*[:：]/i.test(line)){
      flushPending();
      fixed.push(line);
      continue;
    }
...
function ksrParseSmartImport(raw){
  const fixed=ksrAutoFixImportText(raw);
  const parser=(typeof ksrParseQuestionsUltra==="function")?ksrParseQuestionsUltra:(typeof ksrParseQuestionsFull==="function"?ksrParseQuestionsFull:parseBits);
  let qs=parser(fixed)||[];
  ...
  qs=qs.map(q=>({
    ...q,
    q:ksrPrettyQuestionText ? ksrPrettyQuestionText(q.q) : String(q.q||""),
    o:(q.o||["","","",""]).slice(0,4).map(x=>String(x||"").trim()),
    a:Number(q.a)||0,
    subject:q.subject||"General"
  })).filter(q=>q.q && q.o.some(Boolean));

  const expected = Math.max(ksrDetectExpectedCount ? ksrDetectExpectedCount(fixed) : 0, qs.length);
  const issues=[];
  qs.forEach((q,i)=>{
    const missingOpts=(q.o||[]).filter(x=>!x).length;
    if(missingOpts) issues.push({qno:i+1,reason:`${missingOpts} option(s) missing`,raw:q.q});
    if(q.a<0||q.a>3||isNaN(q.a)) issues.push({qno:i+1,reason:"Answer missing/invalid",raw:q.q});
  });
  if(expected && qs.length<expected){
    for(let n=qs.length+1;n<=expected;n++) issues.push({qno:n,reason:"Question not detected from source text",raw:""});
  }
  return {fixed,qs,issues,expected};
}

// Override smart preview render with improved validation and auto fix
function ksrRenderSmartImportPreview(){
  const box=document.getElementById("bitsPreviewBox"), bits=document.getElementById("bits");
  if(!box||!bits)return;
  const {fixed,qs,issues,expected}=ksrParseSmartImport(bits.value||"");
  window.KSR_PREVIEW_QS=qs; window.KSR_MISSING_BLOCKS=issues;
  const ready=issues.length===0 && qs.length>0 && qs.length<=200;
  const subjects={}; qs.forEach(q=>subjects[q.subject||"General"]=(subjects[q.subject||"General"]||0)+1);
  const subjectBadges=Object.entries(subjects).map(([k,v])=>`<span class="import-heading">${ksrSpEsc?ksrSpEsc(k):k}: ${v}</span>`).join(" ");

  box.innerHTML=`<div class="smart-preview-panel">
    <h2>🧠 Smart Import Preview</h2>
    <div class="stat-grid">
      <div class="stat"><div class="label">Expected</div><div class="value">${expected||qs.length}</div></div>
      <div class="stat"><div class="label">Detected</div><div class="value">${qs.length}</div></div>
      <div class="stat"><div class="label">Issues</div><div class="value">${issues.length}</div></div>
      <div class="stat"><div class="label">Limit</div><div class="value">200</div></div>
    </div>
    <div>${subjectBadges}</div>
    ${ready?`<div class="import-ready"><b>✅ READY TO SAVE</b><br>Question count matched. Major issues not found.</div>`:`<div class="import-issue"><b>⚠️ Check Required</b><br>${issues.length} issue(s) found. Fix before final save.</div>`}
    ${qs.length>200?`<div class="smart-bad">⚠️ 200 questions limit exceed ayindi. Current: ${qs.length}</div>`:""}
    <div class="smart-actions">
      <button class="p" type="button" onclick="ksrSavePreviewAndSave()">✅ Save Preview Questions</button>
      <button class="g" type="button" onclick="ksrApplyAutoFix()">🤖 Auto Fix Text</button>
      <button class="g" type="button" onclick="ksrAddQuestionForm()">➕ Add Question</button>
      <button class="s" type="button" onclick="ksrRenderSmartImportPreview()">🔄 Recheck</button>
      <button class="s" type="button" onclick="document.getElementById('bitsPreviewBox').innerHTML=''">Close</button>
    </div>
    <div id="smartAddBox"></div>
    ${issues.length?`<h3>🔎 Problems to Fix</h3>${issues.map((m,i)=>`<div class="smart-bad">
      <b>Issue ${i+1} ${m.qno?`(Q${m.qno})`:""}</b><br>
      <b>Reason:</b> ${(ksrSpEsc?ksrSpEsc(m.reason):m.reason)}
      ${m.raw?`<div class="raw-box">${ksrSpEsc?ksrSpEsc(m.raw):m.raw}</div>`:""}
      <button class="g" type="button" onclick="ksrFixMissing(${i})">Fix & Add This Question</button>
    </div>`).join("")}`:""}
    <h3>📄 Detected Questions</h3>
    <div id="smartQList">${qs.map((q,i)=>ksrQuestionCard(i,q)).join("")}</div>
...
function ksrApplyAutoFix(){
  const bits=document.getElementById("bits");
  if(!bits)return;
  bits.value=ksrAutoFixImportText(bits.value||"");
  ksrRenderSmartImportPreview();
}

// Override preview button to use Smart Import Preview
document.addEventListener("click",function(e){
  if(e.target&&e.target.id==="previewBitsBtn"){
...
// Make save use normalized preview when available
const __ksrOldSavePreviewAndSave = window.ksrSavePreviewAndSave;
window.ksrSavePreviewAndSave=function(){
  if((window.KSR_PREVIEW_QS||[]).length>200) return alert("200 questions limit. Please delete extra questions.");
  if(typeof ksrSyncPreviewToTextarea==="function") ksrSyncPreviewToTextarea();
  document.getElementById("saveExamBtn").click();
};
